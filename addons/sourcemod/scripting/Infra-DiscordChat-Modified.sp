...
{
	name = "Infra's Simple Discord Chat Relay (Modified)",
	author = "sphaxa",
	description = "Simple plugin to relay in-game text chat to a webhook!", 
	version = "1.1.0", 
	url = "https://github.com/sphaxa/SM-Discord-Chat"
};
...
public void OnClientSayCommand_Post(int client, const char[] command, const char[] argText)
{
    ...
    if (g_cvSimpleMode.BoolValue)
	    ...
	    {
    		sendSimpleWebhook(client, messageTxt, false);
	    }
	    else
	    {
	    	sendSimpleWebhook(client, messageTxt, true);
	    }
    else
	    if (!StrEqual(command, "say_team"))
	    {
    		sendWebhook(client, messageTxt, false);
	    }
	    else
	    {
	    	sendWebhook(client, messageTxt, true);
	    }
}

void sendWebhook(int client, char[] text, bool isTeamChat)
{
    ...
    else
    {
        ...
        if (isTeamChat)
	    {
	    	Format(clientName, sizeof(clientName), "%s (Team Chat)", clientName)
	    }
    }
...
void sendSimpleWebhook(int client, char[] text, bool isTeamChat)
{
